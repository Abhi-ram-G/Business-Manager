...
# Validate and parse database connection URL
...
try:
    ...
    if "pooler.supabase.com" in host and host != "aws-0-ap-northeast-1.pooler.supabase.com":
        ...
        # Reconstruct URL with the correct host
        port_part = f":{parsed.port}" if parsed.port else ""
        pass_part = f":{parsed.password}" if parsed.password else ""
        query_part = f"?{parsed.query}" if parsed.query else ""
        DATABASE_URL = f"postgresql://{parsed.username}{pass_part}@aws-0-ap-northeast-1.pooler.supabase.com{port_part}{parsed.path}{query_part}"
        parsed = urlparse(DATABASE_URL)
        host = parsed.hostname
    ...
    if "pooler.supabase.com" in host and username != "postgres.vwtjogybncekikjyqgur":
        ...
        port_part = f":{parsed.port}" if parsed.port else ""
        query_part = f"?{parsed.query}" if parsed.query else ""
        DATABASE_URL = f"postgresql://postgres.vwtjogybncekikjyqgur{pass_part}@{host}{port_part}{parsed.path}{query_part}"
        parsed = urlparse(DATABASE_URL)
        username = parsed.username

    # Print connection details safely (omitting password)
    print("\n" + "="*50)
    print("MIGRATION DATABASE CONNECTION DETAILS:")
    print(f"Host:          {host}")
    print(f"Port:          {parsed.port or 5432}")
    print(f"Database Name: {parsed.path.lstrip('/')}")
    print(f"Username:      {username}")
    print(f"SSL Enabled:   {'sslmode' in parsed.query or 'ssl' in parsed.query or 'sslmode' in DATABASE_URL}")
    print("="*50 + "\n")

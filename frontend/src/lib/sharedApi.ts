...
export const mapLoanGivenFromApi = (item: ApiRecord): LoanGiven => ({
  ...
  interestType: item.interest_type ? String(item.interest_type) : undefined,
...
export const mapLoanReceivedFromApi = (item: ApiRecord): LoanReceived => ({
  ...
  interestType: item.interest_type ? String(item.interest_type) : undefined,
  interestStatus: item.interest_status ? String(item.interest_status) : undefined,
  category: item.category ? String(item.category) : undefined,
  status: item.status ? String(item.status) : undefined,
...
export const fetchSharedSnapshot = async (apiBaseUrl: string) => {
  const [labours, attendance, salaryPayments, vehicles, fuelEntries, trips, loansGiven, loansReceived, familyMembers, incomeEntries, familyExpenses, categoryBudgets, documents, notifications] = await Promise.all([
